...
## Sprint 12 — Quick fixes: numbering, redundancy, tool chrome

From the 2026-07-04 issue dump (items 1, 5, 7, 8, 9, 14). Five small, independent fixes — one session.

- [ ] **Area-agnostic work-area close-up(s)** (item 14) — the "Close-up of the work area" shot lives only in `PHOTO_SPECS.back.shots`, so it's requested only when the back yard is checked and titled "Back yard — Close-up…" even for front-yard work. Promote it to a standalone always-requested shot (own constant + own photo group, the `PHOTO_MATERIAL` pattern; shown whenever at least one area is selected), titled "Close-up(s) of the work area(s)" and **accepting multiple files** — one per work area, since Section 02 can itemize several improvements in different spots. Make the input `multiple` (dropzone.js already appends on multiple inputs — the neighbor-form pattern, incl. page-by-page drops); the `photos` map value for this id becomes a filename *list*, so `collect()`/restore (`is-prior`), the packet row ("N attached"), thumbnails, preview/print, and the mailto manifest all need the plural case. Keep the `back_closeup` id or alias it on restore (a prior single filename restores as a one-item list); give it its own `PHOTO_TITLE` entry (no area prefix); the existing `back_closeup-good/bad.jpg` examples are already generic enough to reuse.

- [ ] **Re-enumerate improvement rows** — `improvementCount` is a monotonic id generator, and the visible `.improvement__num` label uses it, so deleting rows leaves "Improvement 3" as the only item. Add a `renumberImprovements()` that walks `$$(".improvement", improvementList)` and sets each label from its 1-based DOM position; call it after every add and remove. Keep `data-improvement` / input ids / `name`s on the monotonic counter — uniqueness is what the label/`for` pairs, the photo-status wiring, and `updateImprovementStatus(idx)` depend on; only the *display* number re-derives.
- [ ] **Drop the manual sketches attestation** — the `.packet-confirm` checkbox ("Sketches, dimensions, example photos and materials — gathered…") is now redundant: Section 02 itemizes materials + dimensions per improvement and tracks each example picture as a live packet row, and the plot/photos rows are already derived. Remove the checklist item (index.html ~580), `sketchesConfirmed()`, its `packetMissingList()` entry, the `renderPacket()` row, `collect()`'s `req_sketches` + the `SUB_LABELS` entry, and the mailto attach-list line. Old drafts restore harmlessly (the key finds no element — same pattern as the retired `req_fee`).
- [ ] **Instant tooltips on the draw chrome** — tool rail, stamp picker, and Undo/Redo all carry native `title`s today (delayed, easy to miss). Add a small CSS tooltip treatment (`data-tip` attribute + `::after`, no delay; flows right from the rail, above from the status strip's stamp picker) and drop `title` where applied (avoid double tooltips; keep `aria-label`). Redo's tip becomes "Redo (Ctrl+Shift+Z)" — Ctrl+Y still works, the tip should lead with the cross-platform pair Undo advertises.
- [ ] **Paint-bucket icon swap** — replace `ICON.fill` with the user-supplied bucket glyph (`C:\Users\maxam\Downloads\48-fill.svg` — bucket + drop, three stroked paths in a 48-box), restyled to the rail's conventions: inline SVG in the `ICON` map, `viewBox="0 0 48 48"`, `stroke="currentColor"`, `fill="none"`, 17×17 render size, stroke-width scaled to match the neighbors' visual weight (~3.8 at 48-box ≈ 1.9 at 24-box).
