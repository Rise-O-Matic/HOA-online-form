...
let parcelsData = null;       // full-precision parcel GeoJSON (baked extract or live county query)
...
// Pre-baked county parcel extract (assets/parcels.json — every parcel polygon in the
// service-area box plus its situs address, from Riverside County Assessor public records;
// regenerated by tools/fetch-parcels.mjs). Lets address→APN geocoding, the surrounding-
// parcels load, and per-APN geometry lookups resolve instantly in-memory with zero county
// round-trips for the common in-area case; the live ArcGIS endpoints below stay as the
// fallback for addresses outside the baked box (or if this fetch fails).
let parcelData = null;
const parcelDataReady = fetch("assets/parcels.json")
  .then(r => (r.ok ? r.json() : null))
  .then(d => { parcelData = d; if (d && d.parcels) console.info("[parcels] loaded", d.parcels.length, "baked parcels"); return d; })
  .catch(() => null);

// Rebuild a GeoJSON Feature in the exact shape the live county queries return, so
// everything downstream (parcel source, click handler, grid projection, drafts) is
// agnostic about where a parcel came from.
function bakedParcelFeature(rec) {
  const properties = { APN: rec.a };
  if (rec.n != null) properties.STREET_NUMBER = rec.n;
  if (rec.t) properties.SITUS_STREET = rec.t;
  return {
    type: "Feature",
    geometry: { type: rec.m ? "MultiPolygon" : "Polygon", coordinates: rec.g },
    properties
  };
}

// Per-record bounds ([w,s,e,n], computed once and cached on the record) for the
// loadParcels envelope filter.
function bakedParcelBounds(rec) {
  if (!rec._bb) {
...
// [lng, lat] — the community's center of mass: mean of every baked parcel's centroid, equal
// mass per parcel (tools/fetch-parcels.mjs prints the fresh value on regen). initMap prefers
// parcels.json's own `center` field once the extract has loaded; this constant is the fallback.
const HOA_CENTER = [-117.035692, 33.953955];
...
function initMap() {
  ...
  mapInstance = new maplibregl.Map({
    ...
    style: MAP_STYLE,
    center: (parcelData && parcelData.center) || HOA_CENTER,
    zoom: 15,
    scrollZoom: false,
    maxZoom: 20,
  ...
  dragCanvas.addEventListener("pointermove", (e) => {
    ...
    // Shift (momentary) or the Snap checkbox (sticky) makes the rotation stick whenever
    // a property line comes square to the drawing grid — see applyBearingSnap below.
    setRotation(applyBearingSnap(dragStartBearing + dx * ROTATE_DEG_PER_PX, e.shiftKey));
  });
...
async function fetchParcelGeometryByAPN(apn) {
  await parcelDataReady;
  const baked = parcelData?.parcels?.find(p => p.a === apn);
  if (baked) return bakedParcelFeature(baked);
  try {
...
async function loadParcels(lng, lat) {
  const w = lng - PARCEL_RADIUS, s = lat - PARCEL_RADIUS, e = lng + PARCEL_RADIUS, n = lat + PARCEL_RADIUS;

  // Local-first: serve the surrounding parcels straight from the baked extract whenever the
  // whole requested envelope is inside it — the baked records already carry their address
  // properties, so this also replaces the follow-up assessor label query. An envelope that
  // pokes past the baked edge falls through to the live query instead, so the map never
  // shows an artificially truncated neighborhood.
  await parcelDataReady;
  const bb = parcelData && parcelData.bbox;
  if (parcelData?.parcels && bb && w >= bb[0] && s >= bb[1] && e <= bb[2] && n <= bb[3]) {
    const features = parcelData.parcels
      .filter(rec => {
        const r = bakedParcelBounds(rec);
        return r[0] <= e && r[2] >= w && r[1] <= n && r[3] >= s;
      })
      .map(bakedParcelFeature);
    parcelsData = { type: "FeatureCollection", features };
    if (mapStyleLoaded && mapInstance.getSource("parcels")) {
      mapInstance.getSource("parcels").setData(parcelsData);
    } else {
      pendingParcels = parcelsData;
    }
    return;
  }
...
// Shared tail of a successful address\u2192parcel resolution (baked or live): frame the
// neighborhood, load the surrounding parcels, and prompt the click-to-select.
function presentLocatedParcel(parcelFeature, apn, situs) {
  const mapStatus = $("#map-status");

  // Centroid of the matched parcel (outer ring; first polygon if multi)
...
async function locateByAddress(address) {
  ...
  // Local-first: match number + name (+ city, mirroring the live where-clause exactly)
  // against the baked extract \u2014 the common in-area case resolves with no county round-trip.
  // Misses (address outside the baked box, or the extract failed to load) fall through to
  // the live assessor query below.
  await parcelDataReady;
  const baked = parcelData?.parcels?.find(p =>
    p.n === parsed.number && p.s === parsed.name && p.c === "BEAUMONT");
  if (baked) {
    presentLocatedParcel(bakedParcelFeature(baked), baked.a, baked.t || address);
    return;
  }
  ...
  try {
    ...
    presentLocatedParcel(parcelFeature, apn, situs);
  } catch (err) {
...
const STEP_SUBTITLES = {
  1: "Build your plan with our interactive tool, or upload an existing plot plan you already have.",
  2: "Click on your lot to select it. The parcel boundaries are from Riverside County records.",
...
// Step 4 "Back to orientation" — an explicit route back to step 3 next to Clear plan (the
// step dots already offer it, but a labeled button reads clearer). Re-entering Draw with an
// unchanged parcel/bearing doesn't wipe the drawing (rebuildGridForParcel's same-key guard).
$("#plot-back-orient")?.addEventListener("click", () => showStep(3));
...
const rotateFineWrap = $(".rotate-fine");     // the whole pill — carries .is-aligned for the readout
...
// Orientation snapping — hold Shift while dragging (momentary) or tick the Snap checkbox
// (sticky): the rotation sticks whenever a property line comes within SNAP_TOLERANCE_DEG
...
function parcelSnapAngles() {
  const ring = selectedParcelGeoJSON && selectedParcelGeoJSON.geometry
    && selectedParcelGeoJSON.geometry.coordinates[0];
  if (!ring) return [];
  if (ring !== snapAnglesRing) {
    snapAnglesRing = ring;
    snapAnglesCache = computeSnapAngles(ring);
  }
  return snapAnglesCache;
}
function applyBearingSnap(deg, shiftHeld) {
  if (!shiftHeld && !(rotateSnapToggle && rotateSnapToggle.checked)) return deg;
  const snapped = snapBearing(deg, parcelSnapAngles(), SNAP_TOLERANCE_DEG);
  return snapped == null ? deg : snapped;
}
// Ticking the box snaps the current bearing right away when an alignment is in reach —
// otherwise it would only take effect on the next drag, which reads as a dead checkbox.
rotateSnapToggle?.addEventListener("change", () => {
  if (!rotateSnapToggle.checked) return;
  const snapped = snapBearing(parcelBearing, parcelSnapAngles(), SNAP_TOLERANCE_DEG);
  if (snapped != null) {
    userAdjustedRotation = true; // a deliberate orientation choice, like the nudge buttons
    setRotation(snapped);
  }
});
